...
class SQLitePipelineTestCase(unittest.TestCase):
    ...
    def test_process_item(self):
        ...
        software = self.session.query(Product).filter_by(
            product='Facturascripts', version='*', vendor='Facturascripts').first()
        vulnerability = self.session.query(
            Vulnerability).filter_by(cvss_score=4.3).first()
        affected_version = self.session.query(
            AffectedVersionsNumber).filter_by(affected_version_number=1).first()
